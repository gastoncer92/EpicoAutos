...
def Buscar_cliente(conn,idBuscar):
    if idBuscar=='' or idBuscar.strip()=='':
        #print("puta madreeeeee")
        idBuscar="0"
    cursor=conn.cursor()
    #print("asdasdasd")
    #print(type(idBuscar))
    #cursor.execute("SELECT * FROM db_cliente WHERE id_cliente=(?)",(idBuscar,))
    cursor.execute("SELECT * FROM db_cliente WHERE id_cliente=(?) and EXISTS (SELECT * FROM db_cliente WHERE id_cliente=(?))",(idBuscar,idBuscar))  # id_empleado=?
    clientes=cursor.fetchall()
    #print(clientes)
    if not clientes:  #cliente==[]
        clientes=[('','','','')]
    else:pass
    #print("hakuna matataaaa")
    #print(clientes)
    return clientes
